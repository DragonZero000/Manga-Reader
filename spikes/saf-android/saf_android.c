...
int safTakePersistable(uintptr_t jenv, uintptr_t jctx, const char *uri, char **err) {
	...
	(*env)->CallVoidMethod(env, cr, take, u, 1);
	*err = takeException(env);
	return *err == NULL ? 0 : -1;
}

// safIsPersisted — есть ли среди getPersistedUriPermissions() uri с правом чтения.
int safIsPersisted(uintptr_t jenv, uintptr_t jctx, const char *uri) {
	JNIEnv *env = (JNIEnv *)jenv;
	jobject cr = resolver(env, (jobject)jctx);
	jclass crCls = (*env)->GetObjectClass(env, cr);
	...
	jmethodID isRead = (*env)->GetMethodID(env, permCls, "isReadPermission", "()Z");
	jclass uriCls = (*env)->FindClass(env, "android/net/Uri");
