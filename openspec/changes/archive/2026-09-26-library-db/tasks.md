...
- [x] 9.3 GitHub Actions: после коммита на отдельной ветке и push (с разрешения пользователя) — `gh run watch` для CI; оба задания (тесты Windows и сборка APK) зелёные; шаги `release.yml` проверяются локально теми же командами (релизный тег не ставится)
