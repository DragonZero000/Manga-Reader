...
- [x] 6.4 После push на GitHub: `ci.yml` зелёный на ветке
- [x] 6.5 Автор создал ключ и секреты; тег `v0.1.0` → черновик релиза с тремя файлами; `sha256sum -c SHA256SUMS.txt` проходит; zip запускается на Windows с версией `0.1.0`; APK ставится на телефон, а следующий релиз ставится поверх (проверяется при выпуске `0.1.1` или новее). Итог: релиз `v0.1.0` опубликован, суммы и состав файлов проверены, zip и APK проверены автором; установка поверх — при выпуске `0.1.1`
