...
Версия приложения задаётся только в FyneApp.toml в формате MAJOR.MINOR.PATCH; все сборки берут её оттуда, а номер сборки Android (versionCode) вычисляется из версии, чтобы новая версия всегда ставилась поверх предыдущей.
