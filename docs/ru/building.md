...
Сборка с тегом `frameprobe` раз в 5 секунд пишет в журнал, насколько ровно идёт цикл отрисовки: интервалы между кадрами (медиана, p95, максимум) и число кадров дольше 25 мс. В обычных сборках замера нет.

```sh
make build-android EXTRA_TAGS=frameprobe   # APK с замером
adb logcat | grep frameprobe               # frameprobe: n=300 p50=16.4ms p95=17.4ms max=18.1ms >25ms=0
make run EXTRA_TAGS=frameprobe             # то же на ПК (в консоль)
```

Как читать: Fyne рисует по таймеру 60 Гц, поэтому норма — p50 и p95 около 16,7 мс и `n` около 300. Большой `max` и кадры дольше 25 мс означают, что UI-поток занят (например, подготовкой изображений), и искать надо в коде. Замер показывает шаг цикла UI, а не момент показа кадра на экране: если интервалы ровные, а прокрутка всё равно рвётся, причина в подаче кадров на экран. Это проверяют визуально или трейсом Perfetto.

`EXTRA_TAGS` — дополнительные теги сборки через запятую для `run`, `build-windows` и `build-android` (`tools/android-build -tags`).
