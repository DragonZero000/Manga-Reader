...
Вкладка **Поиск** ищет по всей библиотеке. Если строка пуста, под ней показана справка **Как искать**. Условия через пробел объединяются по «И».
...
Даты: `2024` — весь год, `2024-06` — месяц, `2024-06-15` — день; `>период` — после его конца, `<период` — до начала. Если в запросе ошибка, под строкой поиска появится «Ошибка в запросе: …» с пояснением. Результаты обновляются сами, когда меняется библиотека.
